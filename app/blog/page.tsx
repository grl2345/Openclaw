...
export default function BlogPage() {
  ...
  const fetchArticles = useCallback(async (cat: ArticleCategory | "all", off: number, replace: boolean) => {
    setLoading(true)
    ...
    try {
      ...
      // API unavailable (e.g. Supabase not configured yet)
    }
    setLoading(false)
  }, [])
  ...
  useEffect(() => {
    setOffset(0)
    fetchArticles(activeCategory, 0, true)
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <header className="mb-10 text-center">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-sm font-medium text-primary">
              ...
              {isZh ? "原创内容" : "Original Content"}
            </div>
            <h1 className="mb-3 text-3xl font-bold tracking-tight sm:text-4xl">
              {isZh ? "博客 & 教程" : "Blog & Tutorials"}
            </h1>
            <p className="mx-auto max-w-2xl text-lg text-muted-foreground">
              {isZh
                ? "OpenClaw Hub 原创技术博客与实战教程，帮你更快掌握 AI 助理"
                : "Original technical blog posts and hands-on tutorials to help you master AI assistants"}
            </p>
          </header>
